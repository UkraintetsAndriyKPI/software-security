...
class RSACipher:
    ...
    def sign_message(self, message, private_key_str=None):
        """Підписування повідомлення за допомогою приватного ключа"""
        try:
            if private_key_str:
                private_key = RSA.import_key(private_key_str)
            else:
            ...
            raise ValueError(f"Помилка підписування: {str(e)}")

    def verify_signature(self, message, signature, public_key_str=None):
        """Перевірка ЕЦП за допомогою відкритого ключа"""
        try:
            if public_key_str:
                public_key = RSA.import_key(public_key_str)
            else:
            ...
            raise ValueError(f"Помилка перевірки підпису: {str(e)}")
